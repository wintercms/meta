# Build 1.3.0 - Foundation framework upgrade (Laravel 11.x)
...
From the proposed Build 1.3.0 your webserver will require PHP 8.0.2 or above to use Winter CMS. After this date, websites using PHP 7.0-7.2.8 will still function normally but will no longer be able to receive updates or install the latest version.
...
- `config/mail.php`:
    - The structure has changed, update your local copy to match the one [now provided on the 1.2 branch](https://github.com/wintercms/winter/blob/1.2/config/mail.php). The old one should continue working but it is recommended to update it to reflect the new structure.
